...
if (mysqli_num_rows($result) < 1) {
  ...
  while ($row = mysqli_fetch_assoc($result)) {
    ...
    $post_cat = htmlentities($row['post_cat']);
    $des = htmlentities(strip_tags($row['description']));
    $slug = htmlentities($row['slug']);
    $time = htmlentities($row['date']);
    
    $query_cat = "SELECT catname FROM Category where id=".$post_cat;
    $categoryName= mysqli_query($dbcon, $query_cat);
    $cat_name= mysqli_fetch_assoc($categoryName);
    $catname = $cat_name["catname"];
    $permalink = "p/".$id ."/".$slug;
    ...
    echo '</div>';
    echo "<div class='w3-text-grey'>$catname, $time </div>";
    echo '</div>';
}
